...
def get_model_info(year):
    ...
    models_that_year = Model.query.filter(Model.year== year ).all()
    print()
    for this_model in models_that_year:
        print(f'   model name: {this_model.name}')
        print(f'   brand name: {this_model.brand.name}')
        print(f'   brand HQ:   {this_model.brand.headquarters}\n')
...
def get_brands_summary():
    ...
    for brand in Brand.query.all():
        print('\n',brand.name,'summary --------')
        for this_model in brand.models:
            print(f'   {this_model.year} {this_model.name}')
...
def get_models_between(start_year, end_year):
    ...
    return Model.query.filter((Model.year < end_year) 
                              & (Model.year >= start_year)).all()
